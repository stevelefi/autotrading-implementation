...
# By client_event_id (all retries of the same business request)
{compose_project="autotrading"} |= "client_event_id=k-abc-123"
...
1. Get the `client_event_id` or `trace_id` from your request (returned in the HTTP response body or `X-Request-Id` header)
...
```
Each `REPLAY` line shows the `client_event_id` being suppressed. If many different keys are showing `REPLAY` it's a retry storm; if it's the same key repeatedly it's a single client looping.
...
# Follow a client_event_id (business request) across all retries
python3 scripts/trace.py --client-event-id k-abc-123
...
```
2026-03-06T06:30:01.123Z  ingress-gateway-service  [http-nio] INFO  ... trace_id=trc-abc-123 client_event_id=k-abc-123 ...
...
| `client_event_id` | The caller-supplied dedup key. Same key = same logical business request even across retries. Unlike `trace_id`, a retried call shares the same `client_event_id` but gets a new `trace_id`. | Kafka consumers (from event envelope) | `k-abc-123` |
...
trace_id           → ALL log lines for one request, all services
  └─ client_event_id  → was this a NEW, REPLAY, or CONFLICT?
...
**`trace_id` vs `client_event_id`**: `trace_id` is per-attempt (new value each retry); `client_event_id` is per-business-request (shared across all retries). To understand whether a retry went through, filter by `client_event_id` and look for `REPLAY` outcomes.
...
python3 scripts/trace.py --client-event-id k-abc-123 | grep -E "NEW|REPLAY|CONFLICT"
...
python3 scripts/trace.py --client-event-id <key> --service order-service
...
SELECT client_event_id, outcome, created_at_utc
