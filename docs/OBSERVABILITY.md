...
All 8 services set the following MDC fields in every log line. Use them as Loki filter expressions:

| Field | Set by | Example value |
|---|---|---|
| `trace_id` | All services (OTel auto-instrumentation or HTTP header) | `trc-abc-123` |
| `request_id` | HTTP handlers, Kafka consumers | `evt-bbb-456` |
| `idempotency_key` | Kafka consumers (from event envelope) | `k-abc-123` |
| `principal_id` | ingress-gateway-service (from `X-Actor-Id` header) | `user-xyz` |
| `agent_id` | agent-runtime, risk, order (from gRPC request) | `agent-alpha` |
| `signal_id` | agent-runtime, risk (from gRPC request) | `sig-xyz` |
| `order_intent_id` | order-service, ibkr-connector (from gRPC request) | `oi-xyz-789` |
| `instrument_id` | risk, order, ibkr-connector (from gRPC request) | `AAPL` |
