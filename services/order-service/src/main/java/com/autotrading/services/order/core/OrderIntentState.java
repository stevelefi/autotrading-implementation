...
public record OrderIntentState(
  ...
  public OrderIntentState withStatusObserved(OrderLifecycleState next, Instant nowUtc) {
    return new OrderIntentState(orderIntentId, idempotencyKey, createdAtUtc, submissionDeadlineUtc, true, next, stateVersion + 1, brokerSubmitId);
  }

  public OrderIntentState withTimeoutFreeze() {
    return new OrderIntentState(orderIntentId, idempotencyKey, createdAtUtc, submissionDeadlineUtc, false, OrderLifecycleState.UNKNOWN_PENDING_RECON, stateVersion + 1, brokerSubmitId);
  }
}
