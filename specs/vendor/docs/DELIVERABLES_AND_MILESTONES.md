...
|---|---|---|---|---|
| `M0` | Week 1 | Architecture and contract freeze | Trading Core, API/UI, Data Platform, Policy, SRE | Design signoff complete |
| `M1` | Week 1-2 | Local dev foundation + simulator + validation gates | Platform/DevEx, API/UI, QA | Any dev can run local checks |
| `M2` | Week 2-3 | Service skeletons + schema + topics | Trading Core, Data Platform, Broker | Services boot + migrations pass |
| `M3` | Week 3-4 | Signal -> risk -> intent path | Trading Core, Policy | Idempotent intent creation verified |
| `M4` | Week 4-5 | Connector submit + callback normalization | Broker, Trading Core | No duplicate submit/callback effects |
| `M5` | Week 5-6 | Timeout/freeze/reconciliation enforcement | Trading Core, Broker, SRE | 60s timeout path tested end-to-end |
...
| `M8` | Week 8-10 | Paper trading certification and release gates | QA/Release, SRE, All | 10-day paper acceptance criteria met |
| `M9` | Week 10+ | Controlled production rollout | SRE, Trading Core, Ops | Go-live checklist complete |
...
|---|---|---|---|---|
| `D-M0-01` | Current architecture baseline approved | Architecture Group | All team leads | Signed review notes |
...
| `D-M2-03` | Topic bootstrap and environment config | Data Platform | Broker | Bootstrap script logs |

## `M3` to `M5` Core Execution and Consistency
| Deliverable ID | Deliverable | Owner | Reviewer | Evidence |
|---|---|---|---|---|
| `D-M3-01` | Signal -> risk decision path with OPA fail-closed | Policy + Trading Core | QA | Integration test report |
| `D-M4-01` | Order submit/cancel with callback dedupe | Broker | Trading Core | Duplicate-callback test |
...
| Deliverable ID | Deliverable | Owner | Reviewer | Evidence |
|---|---|---|---|---|
| `D-M6-01` | Monitoring API and dashboard live status | API/UI | Ops | UAT signoff |
| `D-M7-01` | Policy deployment and rollback runbook | Policy Platform | SRE | Rollback drill result |
| `D-M8-01` | 10-day paper-trading acceptance report | QA/Release | Product/Ops | Certification report |
| `D-M9-01` | Controlled go-live and rollback checklist | SRE | All leads | Go-live review record |

## Cross-Team Review Gates
| Gate | Trigger | Required Reviewers | Pass Condition |
|---|---|---|---|
| `G1-Contracts` | API/topic/schema change | API/UI + Data + Trading Core | Contract validator passes + reviewers approve |
| `G2-Consistency` | Order flow/state logic change | Trading Core + Broker + SRE | Timeout/freeze/recon tests pass |
| `G3-Operations` | Control/alerts/runbook changes | SRE + Ops + QA | Alert drill and runbook walkthrough complete |
| `G4-Release` | Paper->live promotion | QA + SRE + Product/Ops | All release gates green |
