# Trading Platform Reference Design

Reference design, contracts, reliability controls, and rollout plans for broker-integrated trading.

## Start Here
1. [Start Here](./START_HERE.md)
2. [Trading System Overview](./TRADING_SYSTEM_OVERVIEW.md)
3. [Trading Architecture](./TRADING_ARCHITECTURE.md)
4. [Service Contracts](./SERVICE_CONTRACTS.md)

## Most Used Pages
- [Trading Architecture](./TRADING_ARCHITECTURE.md)
- [Trading Best Practices Baseline](./TRADING_BEST_PRACTICES_BASELINE.md)
- [Order Consistency and Reconciliation](./ORDER_CONSISTENCY_AND_RECONCILIATION.md)
- [API Specification](./API_SPEC.md)
- [Database Schema](./DATABASE_SCHEMA.md)
- [Kafka Event Contracts](./KAFKA_EVENT_CONTRACTS.md)
- [Observability and Alerting](./OBSERVABILITY_AND_ALERTING.md)
- [Testing and Release Gates](./TESTING_AND_RELEASE_GATES.md)

## By Role
- Founder / Ops: [Production Plan](./PRODUCTION_PLAN.md), [Task Tracking Guide](./TASK_TRACKING_GUIDE.md), [Trading Ops Runbook](./runbooks/trading-ops.md)
- Engineering: [Trading Architecture](./TRADING_ARCHITECTURE.md), [Component Interactions](./design/17-component-interactions-and-doc-plan.md), [Component Contract Matrix](./design/19-component-contract-matrix.md)
- Risk / Policy: [Rule Engine OPA](./RULE_ENGINE_OPA.md), [Risk Service Contract](./contracts/risk-service.md), [Policy Bundle Contract](./contracts/policy-bundle-contract.md), [Policy Decision Audit Contract](./contracts/policy-decision-audit-contract.md), [Requirements](./source-of-truth/01-requirements.md)
- SRE / QA / Release: [Deployment and Environments](./DEPLOYMENT_AND_ENVIRONMENTS.md), [Observability and Alerting](./OBSERVABILITY_AND_ALERTING.md), [Testing and Release Gates](./TESTING_AND_RELEASE_GATES.md)

## Source Of Truth Set
- [Requirements](./source-of-truth/01-requirements.md)
- [Reference Architecture](./source-of-truth/02-reference-architecture.md)
- [Domain and Data Contracts](./source-of-truth/03-domain-model-data-contracts.md)
- [Service Contract Catalog](./source-of-truth/04-service-contract-catalog.md)
- [Operations, Security, Reliability](./source-of-truth/05-operations-security-reliability.md)
- [Team Implementation Plan](./source-of-truth/06-team-implementation-plan.md)
- [Traceability Matrix](./source-of-truth/07-traceability-acceptance-matrix.md)
- [Change Control and Versioning](./source-of-truth/08-change-control-versioning.md)
